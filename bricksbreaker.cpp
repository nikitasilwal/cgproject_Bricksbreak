...
void draw() {
    ...
    // Convert the score to a string and display it
    sprintf(buf, "%d", score);
    outtextxy(50, 0, "Score: ");
    outtextxy(100, 0, buf); // Display the score value
}
  ...
  int main() {
    ...
    while (playAgain) {
        ...
        while (1) {
            ...
            if (score == 50 || ball_y >= 490) {
                outtextxy(200, 200, "Game Over");
                outtextxy(200, 230, "Press 'R' to play again or any other key to exit");
                char choice = getch();
